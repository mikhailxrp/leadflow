...
|Таблицы на мобильном (company-зона)|`overflow-x: auto`, горизонтальный скролл|
|Таблицы в платформенной зоне (`< xl`)|Каждая строка → карточка «поле: значение», **без** горизонтального скролла (от 320px). Примитивы `components/platform/MobileCard.tsx`, таблица показывается только `≥ xl` через `hidden xl:block`. Порог именно `xl` (1280px), а не `lg`: сайдбар 220px виден с `lg` (1024px), и на 1024–1279px широкие таблицы не влезают в остаток ширины — там показываем карточки|
