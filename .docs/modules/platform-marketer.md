...
- Маркетолог видит `companyId` своей компании на её карточке (`/platform/companies/:id`, поле «ID» с кнопкой «Скопировать» в `CompanyDetailPageClient.tsx`) — оттуда он передаёт его суперадмину вне системы (чат поддержки и т.п.).
...
| `/today`, `/control`, `/admin/users`, `/admin/settings`, `/admin/pipeline-settings`, `/admin/import` | Нет → redirect на `/leads` |
