# Project Context for Roo Code

Этот файл читается при каждой сессии.
Следуй этим правилам при любой задаче.
Правила лоформдения кода и работы с проектом смотри тут .roo/rules/\*.mdc
...
`**PLATFORM_ADMIN_BOOTSTRAP_*`\*\* используется ровно один раз — скриптом `pnpm bootstrap:platform-admin`, который создаёт первую запись `PlatformAdmin`, если таблица пуста. Дальше платформенные администраторы создаются друг другом через `/platform/admins`, эти переменные можно убрать из `.env` после первого запуска (скрипт не запускается повторно, если хоть одна запись уже есть).
