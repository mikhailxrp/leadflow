...
const METRIKA_OAUTH_SCOPE = 'metrika:offline_data';
const STATE_TTL_MS = 10 * 60 * 1000;

export type MetrikaTokens = {
  accessToken: string;
  refreshToken: string;
  expiresAt: Date;
};

export type MetrikaConnectionStatus = {
  connected: boolean;
  login: string | null;
...
export function buildAuthorizeUrl(companyId: string): string {
  ...
  url.searchParams.set('client_id', requireEnv('YANDEX_METRIKA_OAUTH_CLIENT_ID'));
  url.searchParams.set('redirect_uri', requireEnv('YANDEX_METRIKA_OAUTH_REDIRECT_URI'));
  url.searchParams.set('scope', METRIKA_OAUTH_SCOPE);
...
async function requestTokens(body: URLSearchParams): Promise<MetrikaTokens> {
...
export async function exchangeCodeForTokens(code: string): Promise<MetrikaTokens> {
  return requestTokens(
    new URLSearchParams({
      grant_type: 'authorization_code',
      code,
      client_id: requireEnv('YANDEX_METRIKA_OAUTH_CLIENT_ID'),
      client_secret: requireEnv('YANDEX_METRIKA_OAUTH_CLIENT_SECRET'),
    }),
  );
}
 ...
 */
export async function refreshAccessToken(refreshToken: string): Promise<MetrikaTokens> {
  return requestTokens(
    new URLSearchParams({
...
export async function fetchMetrikaLogin(accessToken: string): Promise<string | null> {
  try {
...
export async function saveMetrikaTokens(
  companyId: string,
  ...
  await prisma.company.update({
    where: { id: companyId },
    data: {
      metrikaAccessToken: tokens.accessToken,
      metrikaRefreshToken: tokens.refreshToken,
      metrikaTokenExpiresAt: tokens.expiresAt,
      metrikaLogin: login,
    },
  });
}
...
/**
 * Очищает все 4 server-only поля токенов Метрики на `Company`.
 * Настройка `settings.yandexMetrika` (counterId/цель) не трогается —
 * она переживает отключение кабинета, как `yandexMode` у Директа.
 */
export async function disconnectMetrika(companyId: string): Promise<void> {
  await prisma.company.update({
    where: { id: companyId },
    data: {
      metrikaAccessToken: null,
      metrikaRefreshToken: null,
      metrikaTokenExpiresAt: null,
      metrikaLogin: null,
    },
  });
}
...
/**
 * Мёржит `counterId`/`qualifiedGoalId` в `Company.settings.yandexMetrika`,
 * не затирая остальные ключи `settings` (assignMode, controlEnabled и т.д.).
 */
export async function saveMetrikaSettings(
  companyId: string,
  input: { counterId: string; qualifiedGoalId: string },
): Promise<void> {
  ...
  const merged = { ...current, yandexMetrika: input };
...
export async function getMetrikaConnectionStatus(companyId: string): Promise<MetrikaConnectionStatus> {
  const company = await prisma.company.findUniqueOrThrow({
    where: { id: companyId },
    select: { metrikaAccessToken: true, metrikaLogin: true, settings: true },
  });

  const settings = parseCompanySettings(company.settings);

  return {
    connected: Boolean(company.metrikaAccessToken),
    login: company.metrikaLogin,
    counterId: settings.yandexMetrika?.counterId ?? null,
    goalConfigured: Boolean(settings.yandexMetrika?.qualifiedGoalId),
  };
}
