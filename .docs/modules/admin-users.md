...
- Настройки уведомлений — `PATCH /api/users/me/notification-preferences`, немедленное сохранение по каждому тумблеру (как `AssignModeSection`), пишет в существующее JSON-поле `User.notificationPreferences` (`{ assignedLead, commentOnLead, reminders }`). Строка «Уведомления в Telegram» в UI задизейблена с пометкой «Появится после подключения Telegram-бота» — реальная привязка `chat_id` через `/start <token>` появится в Phase 13, вручную вводимый `chat_id` сейчас не собирается намеренно.
