...
### Окно атрибуции — 21 день, фиксировано у всех счётчиков (обновлено 2026-07-19)

> **Правка:** ранее этот раздел описывал 16-часовое окно по умолчанию и ручной чекбокс «Увеличенный период учёта конверсий» — это устарело. Яндекс с тех пор изменил продукт: чекбокс убран из интерфейса, 21 день стал стандартным поведением для всех. Проверено по актуальной официальной документации (`yandex.ru/support/metrica/ru/data/offline-conversion-data`, `.../offline-params.html`): «для всех офлайн-данных (офлайн-конверсий, звонков, заказов из CRM) период дополнения визитов составляет 21 день» — без ручной активации.

- Метрика допускает дозапись данных о визите (включая офлайн-конверсию) в течение **21 дня** после его завершения — это фиксированное поведение, ничего не нужно включать ни клиенту, ни нам.
- Прежней «ловушки» (расширение не ретроактивно, нужно включить чекбокс заранее) больше нет как явления — соответствующего чекбокса в интерфейсе Метрики не существует. Инструкция клиенту (Таск 4, `.docs/user-guide/`) не должна просить включать что-либо в Метрике для этого окна.
- 21 день по-прежнему отсчитывается от **последнего визита пользователя** до **начала обработки файла**, не от момента квалификации лида. Cron-батч (решение зафиксировано в `phase-22.5.md`, не событийный триггер) не может гарантировать попадание в окно для лида, квалифицированного спустя много дней после визита — это ограничение остаётся, просто больше не зависит от ручной настройки клиента.
...
- Инструкция клиенту — **оба идентификатора приходят из скрытых полей формы** (`yclid` из URL-параметра, `client_id` через `ym(counterId, 'getClientID', callback)`); система подхватывает их автоматически **только если сайт положил их в отправку заявки** — плоская форма сама URL-параметры не передаёт. Пошаговый разбор для сайта клиента — в разделе ниже «Как добавить поля `yclid` и `client_id` в формы на сайте». Отдельный акцент на 21-дневное окно атрибуции: расширяется вручную в самой Метрике и не ретроактивно (нужно включить заранее).

Полная разбивка по таскам, миграция, DoD — `.docs/phases/phase-22.5.md`.

### Как добавить поля `yclid` и `client_id` в формы на сайте (инструкция для клиента, простыми словами)

> Этот раздел — то, что нужно отдать владельцу сайта. Написан нарочно простым языком: чтобы настроить, не нужно быть программистом. Технический смысл (почему именно эти имена полей и куда они попадают) — в разделах «Источник идентификатора у лида» и «Движок экспорта» выше.

#### Зачем это нужно (в двух словах)

Представь, что каждый посетитель сайта приходит с невидимой биркой на руке. На бирке написано, **откуда он пришёл**. Яндекс.Метрике эта бирка нужна, чтобы понять: «вот этот человек кликнул по рекламе и стал клиентом — покажу такую рекламу другим похожим людям».

Есть две бирки:

- **`yclid`** — бывает только у тех, кто пришёл **по рекламе Яндекс.Директа**. Яндекс сам приписывает её в адрес страницы: `https://ваш-сайт.ru/?yclid=ABC123...`.
- **`client_id`** — есть у **любого** посетителя, у кого на сайте установлена Метрика (это номер, который Метрика выдаёт браузеру).

Проблема: когда человек заполняет форму и жмёт «Отправить», форма отправляет только то, что человек ввёл руками (имя, телефон). Бирки она **сама не отправляет** — их надо в неё положить. Ниже — как это сделать.

#### Шаг 1. Добавь в форму два скрытых поля

«Скрытое поле» — это поле, которое человек не видит, но форма его отправляет вместе с остальными. Внутри своей формы (между `<form>` и `</form>`) добавь ровно эти две строчки:
...
⚠️ **Имена менять нельзя.** Должно быть точно `yclid` и `client_id` (маленькими буквами). Система ищет поля именно с этими именами — назовёшь иначе, ничего не сработает.

#### Шаг 2. Вставь скрипт, который заполнит эти поля

Скрипт — это маленькая «инструкция для браузера»: возьми бирки и впиши их в скрытые поля. Вставь этот код один раз в самый низ страницы, **перед** закрывающим тегом `</body>`:
...
(function () {
  ...
  // 1. Достаём yclid из адреса страницы (…?yclid=ABC123)
  ...
  //    чтобы он не потерялся, если человек походит по другим страницам сайта
  if (yclidFromUrl) {
  ...
  // 4. Вписываем значение во все поля с нужным именем
  ...
  fillField('yclid', yclid);

  // 5. Спрашиваем у Метрики client_id и вписываем его в поле
  if (window.ym) {
    ym(COUNTER_ID, 'getClientID', function (clientID) {
      fillField('client_id', clientID);
    });
  }
})();
</script>
...
В этом коде нужно поменять **только одно** — число `12345678` на **номер своего счётчика Метрики** (тот же, что вбит в карточке «Яндекс.Метрика» на `/admin/integrations`, поле «ID счётчика»). Больше ничего трогать не надо.

#### Шаг 3. Проверь, что заработало (без программистов)

1. Открой свой сайт, дописав в конец адреса `?yclid=test123` — например `https://ваш-сайт.ru/?yclid=test123`, нажми Enter.
...
4. То же самое можно сделать со вторым полем — в нём появится длинный номер от Метрики (это и есть client_id).
5. Верни `type="text"` обратно на `type="hidden"`, чтобы поля снова спрятались.

Если значения появились — всё настроено правильно. Отправь тестовую заявку: в карточке этого лида в Лид-Канале `client_id` будет виден в блоке «Дополнительные поля», а `yclid` уедет в маркетинговые данные лида.

#### Если сайт на Tilda / конструкторе

- В Tilda два скрытых поля добавляются блоком «Форма» → «Скрытое поле» (Hidden field) с именами `yclid` и `client_id`.
- У Tilda есть встроенная передача параметров из адреса в поля формы — тогда `yclid` подставится сам. Но `client_id` всё равно нужно взять у Метрики, поэтому скрипт из Шага 2 (хотя бы его часть про `client_id`) всё равно нужен — вставляется блоком «HTML» (T123).
- В других конструкторах — аналогично: добавить скрытые поля с точными именами и вставить скрипт в блок произвольного HTML.

#### Частые ошибки

| Ошибка | Почему не работает |
| --- | --- |
| Имя поля написали как `yClid`, `client-id`, `clientId` | Система ищет ровно `yclid` и `client_id` (маленькими буквами, через нижнее подчёркивание). |
| Скрипт вставили, но забыли поменять номер счётчика | `client_id` не заполнится — Метрика не поймёт, у какого счётчика спрашивать. |
| Форма на одной странице, а `?yclid=` был на другой (человек походил по сайту) | Без запоминания в cookie (Шаг 2, пункт 2) yclid теряется. В нашем скрипте это уже учтено — просто не удаляй эту часть. |
| На сайте не установлена Метрика | `client_id` взять неоткуда. Сначала на сайте должен стоять счётчик Метрики. |
| Скрытое поле стоит вне тега `<form>` | Оно не отправится вместе с заявкой. Поле должно быть внутри формы. |

> Важно помнить: даже с идеально настроенными полями лид попадёт в Метрику только если у посетителя вообще была хотя бы одна бирка. У человека, который зашёл на сайт напрямую (не по рекламе) и у которого нет Метрики, обеих бирок нет — такой лид примется как обычно, но в Метрику по этому каналу не уедет. Это нормально.
