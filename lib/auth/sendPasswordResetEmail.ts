...
export async function sendPasswordResetEmail({
  ...
  const subject = 'LeadFlow: восстановление пароля';
  const text =
    `Вы запросили восстановление пароля для вашего аккаунта в LeadFlow.\n\n` +
    `Перейдите по ссылке, чтобы задать новый пароль:\n${resetUrl}\n\n` +
    `Ссылка действует 1 час.\n` +
    `Если вы не запрашивали восстановление, просто проигнорируйте это письмо.`;

  const html = `
    <p>Вы запросили восстановление пароля для вашего аккаунта в LeadFlow.</p>
    <p>
