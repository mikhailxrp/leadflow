...
import MarketerProfileClient from '@/components/platform/MarketerProfileClient';
...
async function loadOwnProfile(id: string): Promise<MarketerDetail | null> {
  ...
  if (!marketer || marketer.deletedAt) {
    return null;
  }
...
export default async function PlatformProfilePage() {
  const session = await requirePlatformSession({ roles: ['MARKETER'] });
  const marketer = await loadOwnProfile(session.admin.id);

  if (!marketer) {
    notFound();
  }

  return <MarketerProfileClient marketer={marketer} />;
}
