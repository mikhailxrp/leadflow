...
```
Платформенный администратор   — вне любой компании (PlatformRole: SUPER_ADMIN), создаёт/блокирует
                                 компании, маркетологов и других администраторов, входит в свои
                                 компании через impersonation для поддержки. Не работает с лидами.
...
Роли внутри компании — иерархия (`ADMIN` ⊇ `HEAD` ⊇ `MANAGER`), не три изолированных набора прав: у администратора есть всё, что есть у руководителя, у руководителя — всё, что есть у менеджера. Платформенный уровень (`PlatformAdmin` с ролью `SUPER_ADMIN` или `MARKETER`) — отдельная сущность, не верхушка этой иерархии (он не часть ни одной компании, см. `database.md`). **Платформенные роли между собой — тоже не иерархия**, а два разных скоупа: суперадмин не видит `companyId` компаний маркетолога, маркетолог не управляет платформенными пользователями; проверка — явным списком ролей (`requirePlatformSession({ roles })`), не `hasMinRole` (см. `.docs/modules/platform-marketer.md`).
...
- **Тарифы, биллинг, ЮKassa/Robokassa** — продукт не продаёт себя через себя; коммерция — вне системы. _Уточнение:_ платформенный администратор ведёт **дату продления по офлайн-договору** (`Company.nextPaymentAt`) с email-напоминанием — это **не биллинг** (нет процессинга, тарифов, фичагейтов, самооплаты), а напоминание о сроке; см. `.docs/modules/platform-admin.md`.
...
│   │   ├── admins/                 # Управление платформенными администраторами — только SUPER_ADMIN
│   │   ├── marketers/              # Управление маркетологами (каскадная блокировка, карточка — только просмотр) — SUPER_ADMIN
│   │   ├── activity/               # Активность компаний (+ вкладка маркетологов у SUPER_ADMIN)
│   │   ├── logs/                   # Журнал событий по фильтрам (обе роли, скоуп по видимости)
│   │   └── profile/                # Собственный профиль (ФИО/телефон/соцсети/аватар) — только MARKETER, self-service
│   │
│   └── api/
│       ├── auth/
...
│       │   ├── logs/
│       │   ├── impersonate/
│       │   └── marketer-access/     # end/ — завершение входа маркетолога
│       ├── webhooks/
│       │   ├── tilda/[companyId]/
│       │   ├── wordpress/[companyId]/
│       │   └── leads/
...
# Cron-триггеры платформенного уровня (защита машинных эндпоинтов, напр. дайджест продлений)
CRON_SECRET=
...
# S3-совместимое хранилище (аватары маркетологов) — например, Beget Cloud Storage
S3_ENDPOINT=
S3_REGION=
S3_BUCKET=
S3_ACCESS_KEY_ID=
...
- Никогда не возвращать клиенту: хэши паролей, полные API-ключи, чужие (другого `companyId`) данные; суперадмину — `companyId` компаний маркетологов в платформенных списках.
