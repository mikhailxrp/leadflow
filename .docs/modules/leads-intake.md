...
**Плагины WordPress (и произвольные самописные скрипты) используют разные структуры полей** — единого формата нет, поэтому парсинг устойчив к трём типовым расхождениям:

1. **Вложенность.** Часть плагинов (например, Fluent Forms) оборачивает поля формы во вложенный объект (`{ fields: { email, ... }, source, form_id, entry_id }`) вместо плоского payload. `normalizeLead` рекурсивно флаттенит вложенные объекты (до 4 уровней) ДО сопоставления с `FIELD_ALIASES` — иначе весь вложенный объект целиком попадал бы одним полем в `customFields`, а email/телефон/имя внутри него не распознавались бы вообще. При коллизии ключей (одинаковое имя на разных уровнях вложенности) верхнеуровневое значение сохраняется как есть, вложенное — под путём `родитель.ключ`, ни одно значение не теряется.
2. **Разделённое имя.** Форма может присылать имя и фамилию раздельно (`first_name`/`last_name`, `Имя`/`Фамилия`). `LAST_NAME_ALIASES` (`constants/fieldAliases.ts`) — отдельный набор ключей-фамилий, которые не идут в `customFields`, а склеиваются с найденным именем через пробел.
3. **Непредсказуемые имена ключей.** Конструкторы форм (Fluent Forms и подобные) присваивают полям автосгенерированные системные имена по типу элемента (`input_text`, `input_number`), не по смыслу — угадать по названию ключа, что это «имя», нельзя. Поэтому после сопоставления алиасов и раскладки UTM/marketing применяется резервная эвристика по **форме значения**, не по имени ключа:
   - оставшееся значение, похожее на email (`x@y.z`) → `email`, даже если ключ не совпал ни с одним алиасом;
   - оставшееся значение, похожее на телефон (6–15 цифр, `+`/скобки/пробелы/дефисы допустимы) → `phone`;
   - если после этого осталось **ровно одно** нераспознанное поле, не являющееся служебным (ключ не начинается с `_`, не оканчивается на `_id`/`-id`, не содержит `nonce`/`referer`, не входит в `NON_NAME_META_FIELDS` — `source`, `url`, `timestamp` и т.п.), не пустое, не число и не URL — оно считается именем. Если кандидатов два и больше — имя остаётся `null`, эвристика не гадает.

Все три правила покрывают конкретный воспроизведённый кейс: WordPress-форма на Fluent Forms присылает `{ fields: { email, input_text: "Михаил", description, _wp_http_referer, ... }, source, form_id, entry_id }` — после флаттенинга и эвристик email и имя извлекаются корректно, служебные поля (`_wp_http_referer`, `form_id`, `entry_id`, `source`) остаются в `customFields`, но не могут быть ошибочно приняты за имя. Тесты — `lib/intake/normalizeLead.test.ts`.
