...
> **Статус:** реализовано полностью (Таски 1–4, 2026-07-17). Research (Таск 1) — GO. Миграция + OAuth-флоу подключения счётчика + настройка `counterId`/цели (Таск 2). Движок экспорта + cron + идемпотентность + фолбэк (Таск 3). UI карточки на `/admin/integrations` + инструкция клиенту (Таск 4). Полная бизнес-логика, решения и разбивка по таскам — `.docs/phases/phase-22.5.md`; итоги — `.docs/dev-log.md` → «Phase 22.5».
...
└── YandexMetrikaCard.tsx   # реализовано (Таск 4) — статус/подключение (ADMIN only) + форма counterId/qualifiedGoalId (ADMIN + маркетолог) + инструкция по yclid/client_id

app/(company)/(admin)/admin/integrations/page.tsx  # реализовано (Таск 4) — getMetrikaConnectionStatus(companyId) серверным пропом, без нового клиентского запроса
```

### UI — реализовано (Таск 4, 2026-07-17)

- **`YandexMetrikaCard.tsx`** получает статус серверным пропом со страницы (`getMetrikaConnectionStatus`, тот же паттерн, что `getYandexConnectionStatus` у Директа) — маркетологу не нужен отдельный `GET`-пункт в allow-list, страница уже гейтит вход ADMIN/маркетологом.
- **Подключение/отключение кабинета — ADMIN only**, форма `counterId`/`qualifiedGoalId` — **ADMIN и маркетолог** (симметрично `PATCH`-праву из Таска 2). Это два независимых уровня доступа внутри одной карточки, не единый `readOnly`, как у `YandexDirectCard.tsx` (там `yandexMode` — ADMIN only для всех).
- **Подключение и настройка друг от друга не зависят** — форма `counterId`/цели доступна для редактирования независимо от статуса OAuth-подключения; карточка показывает отдельную текстовую подсказку под каждую из 4 комбинаций (не)подключено × (не)настроена цель.
- Значение `qualifiedGoalId` для предзаполнения формы страница берёт из `settings.yandexMetrika` (уже загружается для `yandexMode`), не из `getMetrikaConnectionStatus()` — та функция отдаёт только производный `goalConfigured: boolean`, не саму строку.
- Инструкция клиенту — `yclid` (подхватывается автоматически), скрытое поле `client_id` (`ym(counterId, 'getClientID', callback)`), отдельный акцент на 21-дневное окно атрибуции: расширяется вручную в самой Метрике и не ретроактивно (нужно включить заранее).
