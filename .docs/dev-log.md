...
## 2026-07-21 — Закрытый лид: только просмотр + нет «следующего действия» + приглушение в списке

**Статус:** ✅ Завершён

**Что было сделано:**

- `lib/leads/closeLead.ts` — в той же транзакции закрытие снимает открытые задачи лида (`TODO`/`IN_PROGRESS` → `CANCELLED`) и `PENDING`-напоминания (→ `CANCELLED`), с событиями `TASK_CANCELLED`/`REMINDER_CANCELLED` (`payload.reason = "LEAD_CLOSED"`) через `tx.event.createMany` — не `writeEvent()`, она пишет мимо транзакции (см. `.docs/modules/reminders.md`, п.3 архитектурных решений). Без этого шага read-only карточка стала бы ловушкой: висящие задачи уже нельзя было бы ни выполнить, ни отменить, а они продолжали бы шуметь в «Сегодня» и стрелять напоминаниями
- `lib/tasks/getNextActions.ts`, `lib/today/getTodayData.ts` (оба запроса задач), `lib/reminders/processReminders.ts` — фильтр `lead: { closeType: null }`. Нужен помимо автоотмены выше — у лидов, закрытых до этого правила, открытые задачи и напоминания остались в БД
- `components/leads/LeadsTable.tsx` — строка закрытого лида `opacity-60` + `hover:opacity-100` (тот же приём, что у `PipelineCard`, но с возвратом к полной непрозрачности — таблицу читают, а не только просматривают); в колонке «Следующее действие» у закрытого лида «—» вместо жёлтого «Нет следующего действия»
- `app/(company)/(app)/leads/[id]/page.tsx` — `isClosed` → поясняющая полоса под шапкой + проброс read-only во все блоки; `canAssign` дополнительно гасится закрытием
- `components/leads/LeadEditForm.tsx` — проп `readOnly`: заголовок «Контактные данные» вместо «Редактировать контакт», кнопка «Редактировать» скрыта, форма недостижима (`!isEditing || readOnly` — на случай, если состояние правки уже было открыто)
- `components/leads/LeadSidebar.tsx` — «Взять в работу» у закрытого лида скрывается, но если лид успели взять — блок «Взято в работу + время» остаётся как факт истории (`closeType === null || hasTakenInWork`)
- `components/leads/CloseLeadMenu.tsx` — выпадашка «Закрыть сделкой / Закрыть отказом» переведена с `absolute` на портал в `document.body` с `fixed`-координатами и переворотом вверх при нехватке места снизу (тот же приём, что в `LeadRowQuickActions`). Раньше у последней строки `/leads` меню упиралось в границы контейнера `overflow-x-auto` и вместо выпадашки появлялась горизонтальная прокрутка. Ширина: по ширине кнопки при `fullWidth` (сайдбар карточки), фиксированные 190px в строке таблицы; `left` клампится по вьюпорту
- `components/tasks/TaskBlock.tsx`, `components/reminders/ReminderBlock.tsx` — проп `readOnly`: кнопки создания скрыты, `canEditTask`/`canManage` возвращают `false` → `TaskItem`/`ReminderItem` уже умеют рендериться без действий (смена статуса кликом по кружку тоже отключается)
- API-гарды `400 LEAD_CLOSED` (по образцу уже существовавших в `/stage` и `/deal-value`): `PATCH /api/leads/:id`, `/assign`, `/take`, `/close` (повторное закрытие переписало бы `closedAt` и `dealValueFinal`), `POST /comments`, `POST /tasks`, `PATCH|DELETE /tasks/:taskId`, `POST /reminders`, `PATCH|DELETE /reminders/:rid`. В `PATCH /api/leads/:id` для этого пришлось добавить `findFirst` перед `updateMany` (раньше факт существования лида выводился из `result.count`)
- `.docs/modules/leads.md` (разделы «Закрытие снимает открытые задачи и напоминания», «Закрытый лид — только просмотр»), `.docs/modules/tasks.md`, `.docs/modules/reminders.md`, `.docs/modules/today.md`

**Решения:**

- **Квалификация остаётся редактируемой у закрытого лида** — сознательное исключение из read-only: маркетолог квалифицирует как раз закрытые лиды (WON/LOST), на воронку это не влияет, и от неё зависит экспорт офлайн-конверсий в Яндекс.Метрику. `PATCH /api/leads/:id/qualification` намеренно **без** гарда `LEAD_CLOSED`
- **Комментарии закрыты вместе со всем остальным** (выбор пользователя из двух вариантов) — карточка закрытого лида ведёт себя как архив, новых записей в ней не появляется
- **Удаление лида (ADMIN) не трогали** — это снос записи целиком, а не правка карточки

**Проверено:** `npm run type-check` — чисто; `npm run lint` — чисто (остался предсуществующий warning в `scripts/seedDemoCompany.ts`, не тронут); `npm test` — 50 тестов, 5 файлов, все проходят.

**Out of scope (не делалось):** переоткрытие лида (по-прежнему нет — при его появлении учесть, что снятые при закрытии задачи сами не воскреснут), показ `dealValueFinal` в сайдбаре закрытого WON-лида (сейчас блок суммы у закрытого лида скрыт целиком — отдельная задача), приглушение закрытых лидов где-либо, кроме `/leads` (на доске воронки оно уже было).
...
- `lib/validations/leads.ts` — `MAX_DEAL_VALUE` экспортирован (была локальная константа), чтобы не заводить второе независимое магическое число под ту же точность `Decimal(14,2)` в новой схеме расхода
- `lib/validations/adSpend.ts` — **НОВОЕ**: `adSpendSchema` (`year` 2020..2100, `month` 1..12, `amountWithVat` неотрицательное число с потолком `MAX_DEAL_VALUE`, `note` — обязательный ключ, `string | null`: `null` — явная очистка заметки, отсутствие ключа не значило бы «не менять», т.к. форма всегда шлёт полную запись); `adSpendQuerySchema` (`from`/`to` опциональны)
- `app/api/ad-spend/route.ts` — **НОВОЕ**: `GET` без query — весь справочник компании для редактора (сортировка по убыванию года/месяца), с `from`/`to` — фильтрация по пересечению `year*12+month` с диапазоном (в JS после одного `findMany`, датасет на компанию мал — без сложных `OR` в Prisma); `POST` — `prisma.adSpend.upsert()` по составному ключу `companyId_year_month`, `createdById` пишется **только в ветке `create`** (по аналогии с `touchIntegrationSource` легко было бы обновлять его в обеих ветках, но семантически это «кто создал запись», не «кто последний правил» — правка чужого месяца не должна переприсваивать авторство), `writeEvent('AD_SPEND_UPDATED', { userId: eventUserId, payload: { year, month, amountWithVat } })` без `leadId`
- `constants/marketerAccess.ts` — `{ pattern: /^\/api\/ad-spend$/, methods: ['GET', 'POST'] }` в `MARKETER_ALLOWED_API` (в отличие от сумм сделки лида в Таске 2, где маркетолог намеренно исключён, здесь он допущен — тот же набор доступа, что у остальных отчётов)
- `types/reports.ts` — тип `AdSpendRecord`
- `components/reports/AdSpendSection.tsx` — **НОВОЕ**: форма месяц/сумма/заметка (`Select` для месяца, `Input` для года/суммы/заметки) + таблица внесённых месяцев с кнопкой «Изменить» (заполняет форму), пустое состояние при пустом справочнике, локальное обновление списка после сохранения без `router.refresh()` (данные не приходят через серверные пропсы страницы — компонент сам себя подгружает)
- `components/reports/ReportsPage.tsx` — новая вкладка «Расходы на рекламу»: `REPORT_NAME_BY_TAB` стал `Partial<Record<ReportTab, ReportExportName>>` (раньше — полный `Record`, обязывал бы запись на каждый таб) — `ExportButton` рендерится только если для активной вкладки есть запись (CSV для этой вкладки — Таск 4); верхний селектор периода отчёта скрыт при активной вкладке (справочник по месяцам не фильтруется period-пикером отчёта — показывать его означало бы вводить в заблуждение)
- `.docs/modules/reports.md` — `GET/POST /api/ad-spend` в таблице эндпоинтов, `AdSpendSection.tsx`/`ad-spend/route.ts` в списке файлов, новый раздел «Расход на рекламу (AdSpend, Phase 22.7)»

**Проверено:** `npm run type-check` — чисто; `npm run build` — чисто, маршрут `/api/ad-spend` зарегистрирован; `npm run lint` — единственный warning в `scripts/seedDemoCompany.ts` (предсуществующий, файл не трогался), без `any`.

**Out of scope (не делалось):** финансовый отчёт, 10 метрик, ROMI, `GET /api/reports/financial`, CSV-экспорт финансового блока — Таск 4; авто-импорт расхода из Яндекс Директа — вне фазы целиком; расход в разрезе источника/канала — вне фазы; отдельный `DELETE /api/ad-spend` — обнуление месяца через `POST` с `amountWithVat: 0`; мультивалютность.

**Definition of Done:** выполнено — все пункты `TASK.md` подтверждены (upsert без дублей, HEAD+/маркетолог допущены, MANAGER — нет, `AD_SPEND_UPDATED` пишется с верной атрибуцией, `createdById` не переприсваивается при правке, UI на `/reports` с пустым состоянием и без CSV-кнопки, type-check/build/lint зелёные).
...
**Контекст:** По запросу пользователя — публичная ссылка вида `APP_URL/demo`, по которой любой желающий попадает в приложение без логина/пароля и может свободно листать все разделы (роль ADMIN), но ничего не может испортить: реальные компании и их данные никак не затрагиваются. Реализовано по ходу диалога, вне `TASK.md`/`phase-N.md` — отдельного модуля в `.docs/modules/` не заводилось.

**Что было сделано:**

- Миграция `add_company_is_demo` — `Company.isDemo Boolean @default(false)`
- `lib/auth.ts` — новый NextAuth Credentials-провайдер `demo-access` (без полей ввода): ищет `Company` с `isDemo: true`, внутри нее — `User` с `role: 'ADMIN'`, возвращает обычную company-сессию с флагом `isDemo`; `jwt`/`session` callbacks прокидывают `isDemo` дальше, как уже сделано для `impersonatedByPlatformAdminId`
- `types/session.ts`, `types/next-auth.d.ts` — `isDemo?: boolean` на `CompanySession.user`, `Session.user` и `JWT`
- `app/(public)/demo/page.tsx` (новый) — публичная страница: на маунте сама вызывает `signIn('demo-access', { redirectTo: '/today' })`. Путь не входит в `matcher` `proxy.ts`, поэтому доступен без сессии автоматически, без правок роутинга
- `lib/prisma.ts` — Prisma Client Extension (`$allOperations` на `$allModels`): любая пишущая операция (`create`/`update`/`delete`/`upsert`/`*Many`), в аргументах которой встречается id демо-компании, бросает `DemoReadOnlyError` до похода в БД — единая точка защиты сразу для всех API-роутов, без правок в каждом из них; id демо-компании кэшируется на процесс (обновится только при рестарте после первого сидирования). Побочный эффект: экспортирован `PrismaTransactionClient`, `lib/roundRobin.ts` переведён на него — тип `tx` внутри `$transaction` расширенного клиента структурно не совпадает с генерируемым `Prisma.TransactionClient`, `npm run type-check` падал без этой правки
- `scripts/seedDemoCompany.ts` (новый, `npm run seed:demo`) — идемпотентно создаёт демо-компанию + 4 пользователей (ADMIN/HEAD/2×MANAGER, переиспользует `hashPassword`) + стандартные этапы/причины отказа (`DEFAULT_STAGES`/`DEFAULT_LOSS_REASONS` из `constants/defaultCompanyData.ts`); при каждом запуске пересоздаёт 12 тестовых лидов (разные источники/этапы/статусы, часть — с комментариями/задачами/напоминаниями). Использует отдельный, не расширенный `PrismaClient` — иначе собственный же guard из `lib/prisma.ts` заблокировал бы сидирование

**Сознательное решение:** демо-сессии выдана роль `ADMIN` не по иерархии прав, а чтобы вся навигация (включая `/admin`, `/reports`, `/team`) была видна и кликабельна. Блокировка мутаций реализована только на уровне БД (Prisma-guard) — визуально кнопки в UI не задизейблены, попытка мутации сейчас всплывает обычным error-тостом («INTERNAL_ERROR»), не тихим disabled-состоянием.

**Out of scope (не делалось):** визуальное отключение мутирующих кнопок и drag-n-drop в канбане под демо-режим (`components/ui/Button.tsx` и т.д.) — отдельная задача, озвучена пользователю; документация в `.docs/modules/`, `CLAUDE.md` — фича не входит ни в одну описанную фазу, отдельного модуля не заведено.

**Проверено:** `npm run type-check` — чисто. Живой прогон на dev-сервере через `curl` (без Playwright): вход через `/demo` → `signIn('demo-access')` → сессия с `isDemo: true`; `GET /api/leads`, `/today`, `/admin/settings` — `200` (демо видит все разделы); `PATCH /api/leads/:id` — заблокирован (`500 INTERNAL_ERROR`, в логе явный `DemoReadOnlyError: blocked Lead.updateMany`), поле в БД не изменилось; попутный `Event.create` (лог открытия лида) тоже блокируется, но не роняет сам `GET` — там уже был try/catch на некритичный побочный эффект. `npm run seed:demo` прогнан дважды подряд — идемпотентен: та же компания/id, пользователи не дублируются, лиды пересозданы. Ветка проверена на конфликты с `main` (`git merge-tree`) — чисто, пересечений файлов с ушедшими вперёд коммитами `main` нет.

**Definition of Done:** формального DoD нет (фича вне `TASK.md`); функционально реализовано и проверено вручную. После деплоя отдельно нужно: применить миграцию (`prisma migrate deploy`) и прогнать `npm run seed:demo` на сервере — без этого `/demo` ответит «временно недоступен».
